  ...
  Future<void> _fetchClientsWithLoans() async {
    ...
    if (userId != null) {
      try {
        ...
        // Obtener todos los préstamos del usuario que deben pagar hoy
        ...
        // Filtrar los clientes cuyos préstamos tienen fecha de pago hoy
        List<String> clientNames = [];
        for (var doc in loanSnapshot.docs) {
          Timestamp fechaNextPayTimestamp = doc['fechaNextPay'];
          DateTime fechaNextPay = fechaNextPayTimestamp
              .toDate()
              .toUtc(); // Convierte a DateTime en UTC
          ...
          // Comparar solo con la fecha actual
          if (fechaNextPayTruncada.isAtSameMomentAs(today)) {
            clientNames.add(doc['clientName']
                as String); // Agrega a la lista si debe pagar hoy
          }
        }
        ...
        // Manejo de errores
        print('Error al obtener clientes con pagos para hoy: $e');
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          : clientsWithLoans.isEmpty
              ? const Center(child: Text('No hay clientes con pagos para hoy.'))
              : ListView.builder(
